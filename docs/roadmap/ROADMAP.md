...
#### 3.8. Настройки проекта для авторизации

**Миграция:** `supabase/migrations/YYYYMMDD_add_auth_settings_to_projects.sql`

```sql
ALTER TABLE projects
ADD COLUMN enable_sign_in BOOLEAN DEFAULT true,
ADD COLUMN enable_sign_up BOOLEAN DEFAULT true;
```

**Обновления:**

- `lib/supabase/types.ts` - добавить поля в типы
- `lib/projects/types.ts` - обновить интерфейс Project
- `components/projects/ProjectFormNew.tsx` - добавить поля в форму
- `config/projects.json` - добавить конфигурацию полей

**Оценка:** 1-2 часа

---

#### 3.9. API Route: Авторизация (Sign In)

**Файл:** `app/api/public/[projectId]/auth/sign-in/route.ts`

**Функционал:**

- Проверка `enable_sign_in` из настроек проекта
...
#### 3.10. API Route: Регистрация (Sign Up)

**Файл:** `app/api/public/[projectId]/auth/sign-up/route.ts`

**Функционал:**

- Проверка `enable_sign_up` из настроек проекта
...
#### 3.11. API Route: Выход (Sign Out)

**Файл:** `app/api/public/[projectId]/auth/sign-out/route.ts`

**Функционал:**

- Выход пользователя через Supabase Auth
- Инвалидация сессии
...
#### 3.12. API Route: OAuth Google (инициирование)

**Файл:** `app/api/public/[projectId]/auth/oauth/google/route.ts`

**Функционал:**

- Генерация OAuth URL через Supabase Admin API
- Возврат `{ redirectUrl: "https://..." }` для редиректа клиента
...
#### 3.13. API Route: OAuth Callback

**Файл:** `app/api/public/[projectId]/auth/callback/route.ts`

**Функционал:**

- Получение `code` из query параметров
- Обмен code на сессию через Supabase
- Создание/обновление профиля пользователя
- Возврат JWT токена и данных пользователя
- Редирект на клиент с токеном (или возврат JSON)

**Оценка:** 3 часа

---

#### 3.14. API Route: Запрос сброса пароля

**Файл:** `app/api/public/[projectId]/auth/forgot-password/route.ts`

**Функционал:**

- Валидация email
- Отправка email для сброса пароля через Supabase
- Возврат успешного статуса (без раскрытия существования email)

**Оценка:** 1 час

---

#### 3.15. API Route: Сброс пароля

**Файл:** `app/api/public/[projectId]/auth/reset-password/route.ts`
...
#### 3.16. API Route: Текущий пользователь

**Файл:** `app/api/public/[projectId]/auth/me/route.ts`
...
#### 3.17. Обработка авторизации в публичном API

**Файл:** `lib/api/public/auth-middleware.ts`
...
- Возврат пользователя и его роли

**Оценка:** 3 часа

---

#### 3.18. Утилита для проверки прав доступа
...
#### 3.19. Документация API endpoints
...
- [ ] Настройки проекта (enable_sign_in, enable_sign_up) работают
- [ ] Эндпоинты авторизации (sign-in, sign-up, sign-out) работают
- [ ] OAuth Google работает (инициирование и callback)
- [ ] Сброс пароля работает
- [ ] Эндпоинт /auth/me работает
- [ ] Документация создана

---

## Фаза 3.5: Система подписок (Subscriptions)

### 🎯 Цель

Создать систему подписок для ограничения доступа к определенным эндпоинтам публичного API на основе активных подписок пользователя.

### 📋 Задачи

#### 3.5.1. EntityDefinition для типов подписок

**Создание через админку или миграцию:**

- EntityDefinition: `subscription_plans`
- Поля:
  - `name` (text) - название подписки
  - `price` (number) - цена
  - `currency` (text) - валюта (USD, EUR, etc.)
  - `interval` (select) - интервал (monthly, yearly)
  - `features` (json) - список возможностей
  - `stripe_price_id` (text, optional) - ID цены в Stripe
  - `is_active` (boolean) - активна ли подписка

**Оценка:** 30 минут (создание через админку)

---

#### 3.5.2. EntityDefinition для экземпляров подписок

**Создание через админку или миграцию:**

- EntityDefinition: `user_subscriptions`
- Поля:
  - `user_id` (relation) - ссылка на пользователя (profiles)
  - `subscription_plan_id` (relation) - ссылка на тип подписки
  - `status` (select) - статус (active, cancelled, expired, pending)
  - `starts_at` (date) - дата начала
  - `ends_at` (date) - дата окончания
  - `stripe_subscription_id` (text, optional) - ID подписки в Stripe
  - `auto_renew` (boolean) - автоматическое продление

**Оценка:** 30 минут (создание через админку)

---

#### 3.5.3. Миграция: Добавить поле в entity_definition

**Файл:** `supabase/migrations/YYYYMMDD_add_subscription_requirements.sql`

```sql
ALTER TABLE entity_definition
ADD COLUMN required_subscription_plan_ids UUID[] DEFAULT ARRAY[]::UUID[];

COMMENT ON COLUMN entity_definition.required_subscription_plan_ids IS
  'Массив ID типов подписок, которые требуются для доступа к этому API';
```
...
#### 3.5.4. Сервис проверки подписок

**Файл:** `lib/api/public/subscription-checker.ts`
...
// Проверить доступ к entityDefinition на основе подписок
...
#### 3.5.5. Интеграция с проверкой разрешений

**Файл:** `lib/api/public/permission-checker.ts`

**Обновление:**

- Добавить проверку подписок после базовых разрешений
- Если `required_subscription_plan_ids` не пусто → проверить активные подписки
- Логика: `базовые разрешения AND (подписка не требуется OR есть активная подписка)`

**Оценка:** 2 часа

---

#### 3.5.6. Обновление публичного API

**Обновить все эндпоинты:**

- `GET /api/public/[projectId]/[entityDefinitionId]`
- `GET /api/public/[projectId]/[entityDefinitionId]/[id]`
- `POST /api/public/[projectId]/[entityDefinitionId]`
- `PUT /api/public/[projectId]/[entityDefinitionId]/[id]`
- `DELETE /api/public/[projectId]/[entityDefinitionId]/[id]`

**Добавить проверку подписок в каждый эндпоинт**

**Оценка:** 4 часа

---

#### 3.5.7. UI для управления подписками (опционально)

**Файлы:**

- `app/projects/[projectId]/subscriptions/page.tsx` - список подписок
- `components/subscriptions/SubscriptionPlanForm.tsx` - форма создания/редактирования плана
- `components/subscriptions/UserSubscriptionList.tsx` - список подписок пользователя

**Оценка:** 1 день (опционально, можно отложить)

---

#### 3.5.8. Интеграция с Stripe (будущее)

**Когда будет готово:**

- Webhook handler для обновления статусов подписок
- Создание подписки после успешной оплаты
- Автоматическое обновление статуса при отмене/возобновлении

**Оценка:** 1-2 дня (когда будет готово)

---

### ✅ Критерии готовности

- [ ] EntityDefinition для типов подписок создан
- [ ] EntityDefinition для экземпляров подписок создан
- [ ] Поле `required_subscription_plan_ids` добавлено в `entity_definition`
- [ ] Сервис проверки подписок работает
- [ ] Интеграция с проверкой разрешений работает
- [ ] Все эндпоинты публичного API проверяют подписки
- [ ] Можно ограничить доступ к API через подписки
